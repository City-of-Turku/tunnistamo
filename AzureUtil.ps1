...
function Get-SecretParametersHashtable {
    return $secretParameters.PSObject.Properties | Where-Object { $_.Name -ne "resourceGroup" } | ForEach-Object -begin {$h=@{}} -process {$h."$($_.Name)" = $_.Value} -end {$h}
}
